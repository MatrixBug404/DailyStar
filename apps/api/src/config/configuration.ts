...
export default () => {
  ...
  if (!publicEndpoint || (!publicEndpoint.startsWith('http://') && !publicEndpoint.startsWith('https://'))) {
    throw new Error('S3_PUBLIC_ENDPOINT is mandatory and must be a valid URL starting with http:// or https://');
  }

  const coverProxyTrustSecret = process.env.PUBLIC_COVER_PROXY_TRUST_SECRET;
  if (!coverProxyTrustSecret || coverProxyTrustSecret.length < 32) {
    throw new Error('PUBLIC_COVER_PROXY_TRUST_SECRET is mandatory and must be at least 32 characters long');
  }
